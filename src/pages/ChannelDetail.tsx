...
const ChannelDetail = () => {
  ...
  const securityData = [{
    name: 'WPA3',
    value: channel.networks.filter(n => n.security === 'WPA3').length,
    color: '#22c55e'
  }, {
    name: 'WPA2',
    value: channel.networks.filter(n => n.security === 'WPA2').length,
    color: '#3b82f6'
  }, {
    name: 'Open',
    value: channel.networks.filter(n => n.security === 'Open').length,
    color: '#ef4444'
  }].filter(item => item.value > 0);
  return <div className="min-h-screen bg-background text-foreground">
      <div className="p-4 md:p-6">
        <div className="mb-6">
          <Button variant="ghost" onClick={() => navigate('/channel-analysis')} className="mb-4">
        ...
        <Card className="mb-6">
          ...
          <CardContent className="bg-gray-950 rounded-md">
          <Collapsible open={metricsExpanded} onOpenChange={setMetricsExpanded}>
            <CollapsibleTrigger className="w-full">
              <div className="flex items-center justify-between">
                ...
                <ChevronDown size={16} className={`transition-transform ${metricsExpanded ? 'rotate-180' : ''}`} />
              </div>
            </CollapsibleTrigger>
            <CollapsibleContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={securityData} cx="50%" cy="50%" labelLine={false} label={({
                    name,
                    value
                  }) => `${name}: ${value}`} outerRadius={80} fill="#8884d8" dataKey="value">
                      {securityData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
        ...
        {/* Alerts */}
        {channel.hiddenNetworks > 0 && <Card className="mt-6">
            <CardHeader>
